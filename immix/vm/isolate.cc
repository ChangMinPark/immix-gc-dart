...
/*
  char VM_ISOLATE[] = "vm-isolate";
  char VM_SERVICE[] = "vm-service";
  char KERNEL_SERVICE[] = "kernel-service";
  char BUILD_KERNEL_SERVICE[] = "kernel_service";
  char BUILD_ISOLATE[] = "isolate";
  ...
  */
  ...
  std::string DEFAULT_ISOLATE[] = {"vm-isolate", "vm-service", "kernel-service", "isolate",
                                ...
                                "kernel_worker.dart", "dartdevc.dart", "pub.dart", 
                                "server.dart" }; 
  bool res = false;

  /* Distinguish Default Isolates. */
  for(int i=0; i < (int) (sizeof(DEFAULT_ISOLATE)/sizeof(std::string)) ; i++){
    if(strncmp(DEFAULT_ISOLATE[i].c_str(), name_prefix, DEFAULT_ISOLATE[i].size()) == 0){
        res = true;
        break;
    }   
  }
